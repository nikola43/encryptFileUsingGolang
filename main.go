...
func decrypt(filename string, key []byte) {
	...
	outfile, err := os.OpenFile("original_"+strings.Replace(filename, ".bin", "", 1), os.O_RDWR|os.O_CREATE, 0777)
	if err != nil {
		log.Fatal(err)
	}
	defer outfile.Close()
...
func encrypt(filename string, key []byte) {
	...
	outfile, err := os.OpenFile(filename+".bin", os.O_RDWR|os.O_CREATE, 0777)
	if err != nil {
		log.Fatal(err)
	}
	defer outfile.Close()
